...
app.post('/api/admin/add-store', protect(['Admin']), async (req, res) => {
    try {
        ...
        if (!address || address.length > 400) {
            return res.status(400).json({ message: "Address is too long (Max 400 chars)." });
        }
...
// Admin: Add New User (Admin, User, or StoreOwner)
app.post('/api/admin/add-user', protect(['Admin']), async (req, res) => {
    try {
        const { name, email, password, address, role } = req.body;

        // Validation
        if (name.length < 20 || name.length > 60) return res.status(400).json({ message: "Name 20-60 chars required" });
        const passRegex = /^(?=.*[A-Z])(?=.*[!@#$%^&*])(?=.{8,16})/;
...
app.patch('/api/admin/users/:id/role', protect(['Admin']), async (req, res) => {
    try {
        const { role } = req.body;
        // Basic check to ensure only valid roles are saved
...
// User: List stores with personal rating
...
app.get('/api/store/my-stats', protect(['StoreOwner']), async (req, res) => {
    try {
        // 1. Find the store where 'owner' matches the logged-in user's ID
        ...
        // 2. Find all ratings for that specific store and populate user names
...
app.get('/api/store/my-stats', protect(['StoreOwner']), async (req, res) => {
    ...
    const raters = await Rating.find({ store: store._id }).populate('user', 'name');
    res.json({ store, raters });
});
